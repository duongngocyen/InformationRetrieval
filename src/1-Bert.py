...
for fold, (train_index, val_index) in enumerate(skf.split(train_df["Content"].values, train_df["sentiment"].values)):
    ...
    # Save the model
    model_path = f"../model/bert-5fold/checkpoint-{fold + 1}"
...
test_df.to_csv(f"../data/OUTPUT_Bert_{NUM_FOLD}fold.csv", index=False)
np.save(f"../data/average_prediction_bert.npy", avg_predictions_prob)