...
training_arguments = TrainingArguments(
    ...
    num_train_epochs=5,
    logging_dir='../logs-distillbert',
    logging_steps=100,
    evaluation_strategy='steps',
    save_steps=500,
    save_total_limit=2,
    load_best_model_at_end=True,
    output_dir="../output-distillbert"
)
...
for fold, (train_index, val_index) in enumerate(skf.split(train_df["Content"].values, train_df["sentiment"].values)):
    ...
    # Save the model
    model_path = f"../model/distillbert-5fold/checkpoint-{fold + 1}"
...
test_df.to_csv(f"../data/OUTPUT_distillBert_{NUM_FOLD}fold.csv", index=False)
np.save(f"../data/average_prediction_distilbert.npy", avg_predictions_prob)